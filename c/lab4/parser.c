...
void statement() {
    ...
    switch(nextToken) {
        ...
        case RUN:
            ...
            // END sets stri=-1 back to file mode and its lex() call already
            // consumed the CR, so we do NOT call lex() here again
            break;
...
void expr_list() {
    if (nextToken == STRING) {
        lexeme[lexLen-1] = 0;         // remove closing quote
        printf("%s\t", lexeme+1);     // print string content (skip opening quote)
        lex(); // advance past the string to look for comma or CR
    } else {
        printf("%d\t",expression());
...
int expression() {
    ...
    while (nextToken == ADD_OP || nextToken == SUB_OP) {
        int op = nextToken;  // save which operator it is BEFORE advancing
